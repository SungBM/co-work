			 ...
			 if(rdata.boardlist.length>0){
				   output += '<li class="comment-order-item ' + red1 + '">'
					...
					$(rdata.boardlist).each(function(){
						const lev = this.comment_re_lev;  //답변글
						let comment_reply = '';
						if(lev==1){
							comment_reply = 'comment-list-item--reply lev1';	//리플라이 레벨 1
						}else if(lev==2){
							comment_reply = 'comment-list-item--reply lev2';	//리플라이 레벨 2
						}
						...
						output += '<li id="' + this.num + '" class="comment-list-item' + comment_reply + '">'  //id=this.num 수정 삭제할라면 num값 있어야됨
							   + '	<div class="comment-nick-area">'		//파일(아이디 앞 사진)이 있는경우 없는 경우
						...
						if($("#loginid").val()==this.id){ //글쓴이만 수정, 삭제 가능하게. admin도 안됨.
							output += '<div class="comment-tool">'
								   + '	<div tittle="더보기" class="comment-tool-button">'
								   + '	  <div>&#46;&#46;&#46;</div>'
								   + '	</div>'
								   + '	<div id="comment-list-item-layer' + this.num + '" class="LayerMore">' //스타일에서 display:none; 설정
								   + '	  <ul class="layer-list">'
								   + '		<li class="layer-item">'
...
function updateForm(num){ //num : 수정할 댓글 글번호
	...
	//수정 폼이 있는 상태에서 더보기를 클릭일 수 없도록 더 보기 영역을 숨겨요
	$(".comment-tool").hide();
	...
	//'.btn-register' 영역에 수성할 글 번호를 속성 'data-id'에 나타내고 클래스 'update'를 추가합니다.
...
function replyform(num,lev,seq,ref){
	...
	//답글 폼의 <textarea>의 속성 'p1aceholder'를 '답글을 남겨보세요'로 바꾸어 줍니다.
	...
	//등록을 답글 완료로 변경합니다.
	$num_next.find('.btn-register').addClass('replay')
...
$(function() {
	...
	$('.comment-area').on('keyup','.comment-write-area-text', function() {
		const length=$(this).val().length;  //text길이를 잰다.
	...
	$('ul+.comment-write .btn-register').click(function() {
		...
		if(!content){ //내용없이 등록을 클릭한 경우
			alert("댓글을 입력하세요");
			return;
		}
		
		$.ajax({
			url : 'CommentAdd.bo', //원문등록
			data : {
				id : $("#loginid").val(),
				content : content,
	...
	$('.comment-area').on('click','.comment-info-button',function(event){
		//답변쓰기 몸이 있는 상대에서 더보기를 클릭할 수 없도록 더 보기 영역을 숨겨요
		$(".comment-tool").hide();
